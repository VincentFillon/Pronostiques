...
$sport = (isset($_POST["sport"])) ? htmlentities($_POST["sport"]) : NULL;
...
if ($sport) {
    ...
    $noms_champs[] = array();
    $query = mysql_query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='championnats'");
    while ($back = mysql_fetch_array($query)) {
       $noms_champs[] = $back["COLUMN_NAME"];
    }
    
    $query2 = mysql_query("SELECT * FROM championnats WHERE Sport=\"" . $sport . "\"");
